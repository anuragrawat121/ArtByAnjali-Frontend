...
const EditArtworkModal = ({ artwork, onSave, onCancel, loading }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-['Mogra'] text-[#D4AF37] mb-6">
          Refine Masterpiece
        </h2>
        <div className="space-y-6">
          <div className="space-y-1">
            <label className="text-[9px] uppercase font-bold tracking-widest text-[#D4AF37]/60 ml-4">
              Title
            </label>
            <input
              type="text"
              className="w-full bg-white/5 border border-white/10 rounded-full px-6 py-3.5 focus:border-[#D4AF37]/30 focus:outline-none text-sm transition-all"
          ...
          <div className="space-y-1">
            ...
            <textarea
              className="w-full bg-white/5 border border-white/10 rounded-[24px] px-6 py-4 h-32 focus:border-[#D4AF37]/30 focus:outline-none resize-none font-['Mogra'] text-sm text-neutral-300 transition-all"
        ...
        <div className="flex gap-4 mt-10">
          <button
            ...
            disabled={loading}
            className="flex-1 bg-[#D4AF37] text-[#1A1512] font-black py-4 rounded-full text-[10px] uppercase tracking-widest hover:bg-white transition-all"
          >
            {loading ? "Saving..." : "Save Changes"}
          </button>
          <button
            onClick={onCancel}
            className="flex-1 bg-white/5 text-neutral-400 font-bold py-4 rounded-full text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all"
          >
            Cancel
          </button>
        </div>
...
const ConfirmModal = ({ onConfirm, onCancel }) => (
  ...
  >
    ...
    >
      ...
      <h2 className="text-2xl font-['Mogra'] text-white mb-2">
        Remove Artwork?
      </h2>
      <p className="text-neutral-500 text-[10px] uppercase tracking-widest leading-relaxed mb-8">
        This masterpiece will be permanently deleted from your cloud studio.
      </p>
      <div className="flex flex-col gap-3">
        <button
          onClick={onConfirm}
          className="w-full bg-red-500 text-white font-black py-4 rounded-full text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all"
        >
          Confirm Delete
        </button>
        <button
          onClick={onCancel}
          className="w-full bg-white/5 text-neutral-400 font-bold py-4 rounded-full text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all"
        >
          Keep it
        </button>
      </div>
...
const AdminDashboard = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      showNotify("Studio Key Invalid", "error");
    } finally {
  ...
  const handleArtworkUpload = async (e) => {
    ...
    const finalCategory = isCustomCategory
      ? newArtwork.customCategory || "Painting"
      : newArtwork.category;
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.message || "Check your image size (Max 80MB)";
  ...
  const handleEditArtwork = async (updatedData) => {
    ...
    try {
      ...
      setArtworks(
        artworks.map((a) =>
          a._id === updatedData._id ? { ...a, ...updatedData } : a,
        ),
      );
  ...
  const addSkill = () => {
    if (skillInput.trim() && !profile.expertise?.includes(skillInput.trim())) {
      setProfile({
        ...profile,
        expertise: [...(profile.expertise || []), skillInput.trim()],
      });
      setSkillInput("");
    }
  };
      ...
      <AnimatePresence>
        {status.show && (
          <StatusNotification
            msg={status.msg}
            type={status.type}
            clear={() => setStatus((p) => ({ ...p, show: false }))}
          />
        )}
        {deleteConfirm.show && (
          <ConfirmModal
        ...
        {editArtworkData && (
          <EditArtworkModal
            ...
            onCancel={() => setEditArtworkData(null)}
      ...
      {!isAuthorized ? (
        <div className="min-h-screen flex items-center justify-center p-6 relative z-10">
          <motion.div
            ...
            className="bg-white/[0.03] border border-white/10 p-8 rounded-[30px] backdrop-blur-xl w-full max-w-sm text-center shadow-2xl"
          >
            <h1 className="text-3xl font-['Mogra'] text-white mb-6">
              ArtByAnjali
            </h1>
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#D4AF37] text-[#1A1512] font-bold py-3 rounded-full text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.2)]"
              >
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <a
                href="/"
                className="text-[9px] uppercase tracking-widest text-white/20 hover:text-white transition-all flex items-center justify-center gap-2 font-black"
              >
        ...
        >
          ...
          <header className="relative z-50 border-b border-white/5 bg-[#0f0f0f]/40 backdrop-blur-xl px-4 md:px-12 py-6 md:py-10 flex flex-col lg:flex-row justify-between items-center gap-8">
            ...
            >
              <div className="w-12 h-12 bg-white text-black rounded-xl flex items-center justify-center shadow-2xl shadow-white/5">
                <Brush size={24} />
              </div>
              <h1 className="text-2xl md:text-3xl font-['Mogra'] text-white tracking-tighter">
                ArtByAnjali
              </h1>
            </motion.div>
              ...
              {[
                ...
                >
                  {activeTab === tab.id && (
                    <motion.div
                      layoutId="nav-bg"
                      className="absolute inset-0 bg-[#D4AF37] rounded-full z-0 shadow-[0_0_20px_rgba(212,175,55,0.3)]"
                    />
                  )}
            ...
            <div className="flex items-center gap-8 lg:gap-12">
              <a
                href="/"
                target="_blank"
                className="text-[10px] md:text-[11px] uppercase tracking-[0.2em] text-white/40 hover:text-white transition-all flex items-center gap-3 group font-bold"
              >
                <ExternalLink
                  size={14}
              ...
              <motion.button
                ...
                className="text-[10px] md:text-[11px] uppercase tracking-[0.2em] transition-all font-bold"
              >
                Sign Out
              </motion.button>
            </div>
          ...
          <main
            className="relative z-10 flex-1 p-6 md:p-10 max-w-[1300px] mx-auto w-full"
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === "artworks" && (
                ...
                >
                  ...
                  >
                    <h1 className="text-3xl font-['Mogra'] mb-1 text-[#D4AF37]">
                      New Exhibition
                    </h1>
                    <p className="text-white/20 text-[9px] uppercase tracking-[0.5em] pl-1">
                      Capture inspiration
                    </p>
                  </motion.div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="space-y-6">
                          <div className="space-y-1">
                            <label className="text-[9px] uppercase font-bold tracking-widest text-[#D4AF37] ml-4 opacity-70">
                              Title
                            </label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1 relative z-[60]">
                            ...
                            <div
                              ...
                              className="w-full bg-white/5 border border-white/10 rounded-full px-6 py-3.5 flex justify-between items-center cursor-pointer text-sm hover:border-white/20 transition-all"
                            >
                              <span className="text-neutral-400">
                                {isCustomCategory
                                  ? newArtwork.customCategory || "Custom..."
                                  : newArtwork.category}
                              </span>
                              <ChevronDown
                                size={14}
                            ...
                            <AnimatePresence>
                              {isDropdownOpen && (
                                <motion.div
                                  ...
                                  className="absolute top-full left-0 w-full mt-2 bg-neutral-900 border border-white/10 rounded-[24px] p-2 z-[999] shadow-2xl backdrop-blur-3xl max-h-64 overflow-y-auto no-scrollbar"
                                >
                                  {categories.map((cat) => (
                                    <button
                                      key={cat}
                                      ...
                                      className={`w-full text-left px-5 py-3 rounded-xl text-[11px] transition-all ${cat === "custom" ? "text-white font-bold border-b border-white/5 mb-1" : "text-neutral-500 hover:text-white hover:bg-white/5"}`}
                                    >
                                      {cat === "custom" ? "+ New Folder" : cat}
                                    </button>
                                  ))}
                                </motion.div>
                              )}
                          ...
                          {isCustomCategory && (
                            <motion.input
                              ...
                              className="w-full bg-white/10 border border-white/20 rounded-full px-6 py-3.5 focus:outline-none text-sm"
                              value={newArtwork.customCategory}
                              onChange={(e) =>
                                setNewArtwork({
                                  ...newArtwork,
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              className="w-full bg-white/5 border border-white/10 rounded-[24px] px-6 py-6 h-32 focus:border-[#D4AF37]/30 focus:outline-none resize-none font-['Mogra'] text-sm text-neutral-300 leading-tight transition-all"
                        ...
                        <div className="flex flex-col gap-6">
                          <div className="flex-1 min-h-[200px] border-2 border-dashed border-white/5 rounded-[30px] flex items-center justify-center relative hover:border-white/10 transition-all overflow-hidden bg-white/5 group">
                            {artworkFile ? (
                              ...
                              <div className="text-center opacity-20 group-hover:opacity-40 transition-opacity">
                                ...
                                <p className="text-[10px] font-['Mogra'] tracking-widest uppercase">
                                  Canvas
                                </p>
                              </div>
                            )}
                          ...
                          <div className="relative">
                            <span className="absolute left-8 top-1/2 -translate-y-1/2 text-[#D4AF37] font-bold">
                              ₹
                            </span>
                            <input
                              type="text"
                              className="w-full bg-white/5 border border-white/10 rounded-full px-12 py-3.5 text-center font-bold text-white focus:outline-none transition-all"
                          ...
                          <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            disabled={loginLoading}
                            className={`w-full font-bold py-4 rounded-full text-[11px] uppercase tracking-[0.3em] transition-all shadow-lg shadow-white/5 ${loginLoading ? "bg-white/10 text-white/40" : "bg-[#D4AF37] text-[#1A1512]"}`}
                          >
                            {loginLoading
                              ? "Publishing..."
                              : "Publish Masterpiece"}
                    ...
                    <div className="lg:col-span-4 flex lg:flex-col gap-6">
                      ...
                      >
                        <motion.h4
                          ...
                          className="text-5xl font-['Mogra'] text-[#D4AF37] leading-none"
                        >
                          {artworks.length}
                        </motion.h4>
                        <p className="text-[10px] uppercase tracking-widest text-[#E8D5C4] mt-1 font-bold">
                          Exhibits
                        </p>
                      </motion.div>
                      <motion.div
                      ...
                      >
                        ...
                        >
                          <Sparkles
                            size={20}
                            className="text-[#D4AF37]/40 mb-2"
                          />
                        </motion.div>
                        <p className="text-[8px] font-['Mogra'] uppercase tracking-widest text-[#E8D5C4]/40">
                          Studio Storage
                        </p>
                      </motion.div>
                    </div>
                    ...
                    {artworks.map((art, idx) => (
                        ...
                        >
                          <img
                            ...
                            className="w-full h-full object-cover transition-all duration-[2s] ease-out opacity-0 scale-95 blur-md grayscale-0 md:grayscale md:group-hover:grayscale-0 group-hover:scale-110"
                          />
                        <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none">
                          ...
                          <h4 className="font-['Mogra'] text-[11px] text-[#E8D5C4] truncate uppercase tracking-tighter">
                            {art.title}
                          </h4>
                        </div>
                        <div className="absolute top-4 right-4 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all z-20">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setEditArtworkData(art);
                            }}
                            className="w-9 h-9 bg-black/60 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-[#D4AF37] hover:bg-white/5 transition-all"
                          >
                            <PlusCircle size={14} />
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setDeleteConfirm({ show: true, id: art._id });
                            }}
                            className="w-9 h-9 bg-black/60 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-red-400 hover:bg-red-500/20 transition-all"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
              ...
              {activeTab === "profile" && (
                ...
                >
                  ...
                  >
                    <h1 className="text-3xl font-['Mogra'] mb-1">
                      Atelier Identity
                    </h1>
                    <p className="text-neutral-600 text-[9px] uppercase tracking-[0.5em]">
                      The soul behind user
                    </p>
                  </motion.div>
                  <motion.form
                    variants={sectionVariants}
                    onSubmit={handleProfileUpdate}
                    className="flex flex-col-reverse md:grid md:grid-cols-2 gap-10 bg-[#1A1512] p-8 md:p-10 rounded-[40px] border border-white/5"
                  >
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[9px] uppercase tracking-widest text-[#D4AF37] ml-4 font-bold opacity-70">
                          Expertise Palette
                        </label>
                            ...
                            {Array.isArray(profile.expertise) &&
                              profile.expertise.map((skill) => (
                                <motion.span
                                  ...
                                  key={skill}
                                  className="px-4 py-1.5 bg-white/10 border border-white/5 text-white text-[9px] uppercase font-bold rounded-full flex items-center gap-2 group transition-all text-nowrap"
                                >
                                  {skill}
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setProfile({
                                        ...profile,
                                        expertise: profile.expertise.filter(
                                          (s) => s !== skill,
                                        ),
                                      })
                                    }
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-white/5 border border-white/10 rounded-full px-6 py-3 text-sm focus:border-white/20 focus:outline-none transition-all"
                          ...
                          <button
                            type="button"
                            onClick={addSkill}
                            className="w-11 h-11 bg-white text-black rounded-full flex items-center justify-center font-bold text-lg hover:scale-105 transition-transform"
                          >
                            +
                          </button>
                        </div>
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] uppercase tracking-widest text-neutral-600 ml-4 font-bold">
                          Bio
                        </label>
                        <textarea
                          className="w-full bg-white/5 border border-white/10 rounded-[24px] px-6 py-6 h-56 font-['Mogra'] text-sm text-neutral-400 focus:outline-none leading-tight resize-none transition-all"
                          value={profile.bio || ""}
                          onChange={(e) =>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input
                          type="email"
                          className="w-full bg-white/5 border border-white/10 rounded-full px-6 py-3 text-sm transition-all"
                          value={profile.email || ""}
                          onChange={(e) =>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="relative flex items-center">
                          <Instagram
                            className="absolute left-6 text-white/20"
                            size={14}
                          />
                          <input
                            type="text"
                            className="w-full bg-white/5 border border-white/10 rounded-full pl-14 pr-6 py-3 text-[11px] transition-all"
                      ...
                      <motion.button
                        ...
                        className="hidden md:block w-full bg-white text-black font-black py-4 rounded-full uppercase tracking-widest text-[11px] shadow-lg shadow-white/5"
                      >
                        Save Atelier
                      </motion.button>
                    </div>
                    <div className="flex flex-col items-center justify-center space-y-8">
                      <motion.div
                        whileHover={{ rotate: 2 }}
                        className="w-48 h-48 sm:w-60 sm:h-60 rounded-full border-4 border-dashed border-white/5 flex items-center justify-center relative overflow-hidden group shadow-xl shimmer-container"
                      >
                        ...
                        <div className="flex justify-center gap-3">
                          {["top", "center", "bottom"].map((pos) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                setProfile({ ...profile, imagePosition: pos })
                              }
                              className={`relative px-6 py-2 rounded-full text-[9px] uppercase tracking-widest transition-all ${profile.imagePosition === pos ? "text-black font-black" : "text-neutral-600 hover:text-white"}`}
                            >
                              {profile.imagePosition === pos && (
                                <motion.div
                                  layoutId="pos-bg"
                      ...
                      <motion.button
                        whileHover={{ scale: 1.01 }}
                        className="md:hidden w-full bg-white text-black font-black py-4 rounded-full uppercase tracking-widest text-[11px]"
                      >
                        Save Atelier
                      </motion.button>
                    </div>
              ...
              {activeTab === "messages" && (
                ...
                >
                  <motion.div
                    variants={itemVariants}
                    className="mb-8 flex flex-col sm:flex-row justify-between items-center sm:items-end gap-6"
                  >
                  ...
                  <div className="space-y-6">
                    {messages.length === 0 ? (
                      <motion.div
                        variants={itemVariants}
                        className="p-20 text-center text-neutral-800 border-2 border-dashed border-white/5 rounded-[40px]"
                      >
                      ...
                      messages.map((m) => (
                        <motion.div
                          variants={itemVariants}
                          key={m._id}
                          className="p-8 bg-[#1A1512] border border-white/5 rounded-[40px] group relative overflow-hidden"
                        >
                          {!readMessages.has(m._id) && (
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/[0.02] flex items-center justify-center rotate-45 translate-x-16 -translate-y-16 pointer-events-none">
                              <span className="text-[7px] font-black uppercase text-white/20 tracking-tighter -rotate-45 mt-16">
                                New Echo
                              </span>
                            </div>
                          )}
                          ...
                          <p className="font-['Mogra'] text-lg text-neutral-300 leading-snug pl-6 border-l-2 border-white/10 group-hover:border-white/30 transition-colors">
                            "{m.message}"
                          </p>
                        </motion.div>
                      ))
          ...
          <footer className="relative z-50 py-12 text-center border-t border-white/5 mt-20">
            <h2 className="text-lg font-['Mogra'] text-[#D4AF37] tracking-[0.2em]">
              ArtByAnjali
            </h2>
            <p className="text-[8px] uppercase tracking-[0.4em] mt-2 text-white/20">
              Noir Atelier Console
            </p>
            <div className="mt-8 pt-8 border-t border-white/5 inline-block px-10">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[7px] uppercase tracking-[0.4em] text-[#D4AF37]/60 hover:text-[#D4AF37] transition-all font-normal"
              >
